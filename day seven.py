...
def solution_part_two(input_path):
    ...
    smallest_poss = 100000000000
    target = 70000000-find_dir_size('home//')
    target = (30000000-target)
    for dir in visited_dirs.keys():
        if find_dir_size(dir) > target:
            if find_dir_size(dir) < smallest_poss:
                smallest_poss = find_dir_size(dir)
